"""Train 07b with a larger source-balanced train split and a held-out test split."""
...
for mixture_entry in namespace["MIXTURE"]:
    mixture_entry["train_count"] = namespace["TRAIN_PER_SOURCE"]
    mixture_entry["val_count"] = namespace["VAL_PER_SOURCE"]
...
config, _ = namespace["build_config"]("B_more_data", use_static_context=True)
config["force_train"] = True
config["force_train_autoencoder"] = False
config["pretrained_ae_cache_path"] = str(
    namespace["OUTPUT"]
    / "mixed4source_A_no_context_latent8_attention_stored13_unbalancedAE_train20_100frames.pt"
)
config["pretrained_ae_require_matching_normalizers"] = False
...
config["propagator_config"]["checkpoint_metric"] = "val_rollout_macro_source_endpoint_p_ratio_r2"
...
config["cache_path"] = str(
    namespace["OUTPUT"] / "mixed4source_B_more_data_stored13_train70_100frames.pt"
)
