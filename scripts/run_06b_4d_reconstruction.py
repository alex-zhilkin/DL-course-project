"""Documented 06b 4D recipe reconstructed on protected matched splits."""
import argparse
import hashlib
import json
import os
...
def main():
    ...
    output=ROOT/f'notebooks/results/06b_4d_reconstruction/d4_s{args.seed}'
    ...
    ae=dict(model='attention',latent_dim=4,latent_tokens=32,hidden_size=96,
    ...
    (output/'completed.json').write_text(json.dumps(dict(seconds=time.time()-started,job_id=os.environ['PBS_JOBID'],split='validation',latent_dim=4)))
