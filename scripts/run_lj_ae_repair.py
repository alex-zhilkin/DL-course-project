"""AE-only source-wise repair study; mixed-T is evaluation-only."""
import argparse
import copy
import hashlib
import json
...
VARIANTS=('longer','equal_source','response_selection','wider','dimension4',
          'dimension4_response','lj_edges','lj_edges_response')
...
def main():
    ...
    output=ROOT/f'notebooks/results/lj_ae_repair/{args.variant}_s{args.seed}'
    ...
    (output/'recipe.json').write_text(json.dumps(dict(source=source,config=cfg,mixed_evaluation=mixed_source,
        ...
        contrasts={'longer':'Original node-pooled objective/field selection; increase cap14->40 and patience3->8',
