"""Train only the 07b propagator against an already trained stored-edge AE."""
...
config, _ = namespace["build_config"]("B_closedloop10_progress_conditioned", use_static_context=True)
...
config["propagator_config"]["fixed_history_include_progress"] = True
...
config["cache_path"] = str(AE_CACHE.with_name("mixed4source_B_progress_conditioned_stored13_train20_100frames.pt"))
config["rollout_final_eval_sims_per_source"] = 10
...
print(result["dyn_history"].tail().to_string(index=False), flush=True)
